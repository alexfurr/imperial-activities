<?php
$imperialDrawingCPT = new imperialDrawingCPT();
class imperialDrawingCPT
{
...
/*	---------------------------
	...
	function addWPActions ()
	{
		...
		// Add metaboxes
		add_action( 'add_meta_boxes_imperial_canvas', array( $this, 'addMetaBoxes' ));
		
		// Remove and add columns in the admin table
		add_filter( 'manage_imperial_canvas_posts_columns', array( $this, 'my_custom_post_columns' ), 10, 2 );		
		add_action('manage_imperial_canvas_posts_custom_column', array($this, 'customColumnContent'), 10, 2);	
...
/*	---------------------------
	...
	function create_CPT ()
	{
		...
		register_post_type( 'imperial_canvas', $args );
	}
	...
	function  drawContentMetaBox($post,$callbackArgs)
	{
		...
		// Add Nonce Field
		wp_nonce_field( 'save_imperial_canvas_metabox_nonce', 'imperial_canvas_metabox_nonce' );
	...
	function savePostMeta ( $postID )
	{
		...
		if($post_type=="imperial_canvas")
		{
			

		
			// Check if nonce is set.
			if ( ! isset( $_POST['imperial_canvas_metabox_nonce'] ) ) {
			
				return;
			}
			
			// Verify that the nonce is valid.
			if ( ! wp_verify_nonce( $_POST['imperial_canvas_metabox_nonce'], 'save_imperial_canvas_metabox_nonce' ) ) {
